...
const ReleaseOverview = ({ release }) =>
  <div className='table-row' id={ anchorSafeString([ 'release', release.get('timestamp') ]) }>
    <div className='row thin'>
      <h3 className='col-6'>Release Overview</h3>
      <div className='col-6 text-right'>
        <button className='with-glyph glyph-x-action-color' onClick={ release.destroy }>Delete release</button>
      </div>
    </div>
...
const ReleaseVariables = ({ env }) =>
  <div>
    <h5>Env Variables</h5>
    { env.count() == 0 && (<p className='text-note'>No environment variables set.</p>) }
    { env.count() > 0 && (
      <table>
        <thead>
          <tr>
...
const ReleaseMounts = ({ mounts = fromJS([]) }) =>
  <div>
    <h5>Mounts</h5>
    { mounts.count() == 0 && (<p className='text-note'>No volumes defined.</p>) }
    { mounts.count() > 0 && (
      <table>
        <thead>
          <tr>
...
const ReleaseDetail = ({ release }) =>
  <div className='release-detail'>
    {/*
    <div className='release-detail-menu'>
      <ReleaseOverviewMenu release={ release } />
      <ReleaseContainersMenu containers={ release.get('containers') } release={ release } />
      <ReleaseVolumesMenu volumes={ release.get('volumes') } release={ release } />
    </div>
    */}
    <div className='release-detail-show'>
      <ReleaseOverview release={ release }/>
      <ReleaseContainers containers={ release.get('containers') } release={ release }/>
