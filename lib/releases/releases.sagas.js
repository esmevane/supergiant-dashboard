...
function* refreshReleases({ appName, componentName }) {
  ...
  yield put(ReleaseActions.fetch(appName, componentName))
  yield put(ReleaseActions.getCurrent(appName, componentName))
  yield put(ReleaseActions.getTarget(appName, componentName))
}
...
function* deleteRelease({ id, appName, componentName }) {
  ...
  if (response.status === 200) {
    ...
    yield put(ReleaseActions.clear(appName, componentName))
...
function* clear(getState) {
  yield takeEvery(ReleaseActions.Clear, refreshReleases)
}

export function* all(getState) {
  yield fork(clear, getState)
