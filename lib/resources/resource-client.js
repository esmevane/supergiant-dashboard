...
export const throwErrorUnless = successCode =>
  ({ response, body }) => {
    const { status, url } = response
    const noMatch = status !== successCode
    const hasNumber = successCode.indexOf && successCode.indexOf(status) > -1
    const errors = body.errors ? body.errors.join(', ') : body.error

    if (hasNumber) { return { response, body } }
    if (noMatch) { throw new Error(`[${status}] '${url}': ${errors}`) }
...
export default class ResourceClient {
  ...
  *[Symbol.iterator]() {
  ...
  delete(key) {
    ...
    return destroy(key).then(throwErrorUnless([200, 202]))
  }
