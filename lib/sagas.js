...
import * as DeployEntity from './deploys/deploys.entity'
...
import * as RegistryEntity from './registries/registries.entity'
...
function* destroyApp() {
  while(true) {
    ...
    if (response.status === 202) {
      ...
      yield put(NotificationActions.add('info', `Deleted app ${id}`))
      yield put(push(`/`))
    } else {
      if (body.errors) {
        yield fork(errorMessages, ...body.errors)
...
function* createDeploy() {
  while(true) {
    ...
    if (response.status === 201) {
      yield put(NotificationActions.add(
        `info`,
        `Deploying ${appName}/${componentName}`
      ))
    } else {
      if (body.errors) {
        yield fork(errorMessages, ...body.errors)
...
function* destroyEntrypoint() {
  while(true) {
    ...
    if (response.status === 200) {
      ...
      yield put(NotificationActions.add('info', `Deleted entrypoint ${id}`))
      yield put(push(`/`))
    } else {
      if (body.errors) {
        yield fork(errorMessages, ...body.errors)
...
function* destroyRegistry() {
  while(true) {
    ...
    if (response.status === 200) {
      ...
      yield put(NotificationActions.add('info', `Deleted registry ${id}`))
      yield put(push(`/`))
    } else {
      if (body.errors) {
        yield fork(errorMessages, ...body.errors)
...
function* errorMessages(...messages) {
  for (let message of messages) {
    yield put(NotificationActions.add('error', message))
  }
}
