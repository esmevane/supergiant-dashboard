...
function apps(state = fromJS({}), action) {
  ...
  switch (action.type) {
    ...
    case AppActions.Insert:
      return state.set(action.app.get('name'), action.app)
    ...
    case ComponentActions.Destroy:
      return state.set(
        app.get('name'),
        app.set('components',
          componentIds.splice(componentIds.indexOf(action.id), 1)
        )
      )
    default:
      return state
...
function clouds(state = fromJS({}), action) {
  ...
  switch (action.type) {
    case AppActions.Insert:
      ...
      return state.set(
        cloud.get('name'),
        cloud.set('apps', apps.push(action.app.get('name')))
      )
...
function components(state = fromJS({}), action) {
  switch (action.type) {
    case ComponentActions.Insert:
      return state.set(action.component.get('name'), action.component)
    case ComponentActions.Destroy:
